...
def video_upload():
    try:
        ...
        # clear s3  of all videos
        deleteVideo()
        # Upload the video to S3 without saving it locally
        for file in file_objects:
            if file.filename == '':
                print("no filename")
                continue

            # You can customize the S3 key (file name) here
            custom_filename = "vid.mp4"

            # Upload the file object directly to S3
            s3.upload_fileobj(file, os.environ.get('BUCKET_NAME'), custom_filename)
